...
//轮函数f
//函数f(R, K)接受两个输入：32 位的数据和 48 位的子密钥。然后,通过表 E 进行扩展置换，将输入的 32 位数据扩展为 48 位
//将扩展后的 48 位数据与 48 位的子密钥进行异或运算
//将异或得到的 48 位数据分成 8 个 6 位的块，每一个块通过对应的一个 S 表产生一个 4 位的输出。其中，每个 S 表都是 4 行 16 列。具体的置换过程如下
//把 6 位输入中的第 1 位和第 6 位取出来行成一个两位的二进制数 x ，作为 Si 表中的行数（0~3）；把 6 位输入的中间 4 位构成另外一个二进制数 y，作为 Si 表的列数（0~15）
...
//尾置换IP^-1
//合并 L16 和 R16 得到一个 64 位的数据，再经过尾置换后得到的就是 64 位的密文。
...
//在这里调用DES
void DESInterface(string s,string k) {
	...
	ifstream in;
	ofstream out;
	out.open("D://tests//des//plaintext.txt", ios::binary|ios::ate);
	bitset<64> plain;
	int turn = s.length() / 8; if (s.length() % 8 != 0)turn++;
	for (int i = 0; i < turn; i++) {
		string a = s.substr(i * 8, 8);
		//字符填充
		while (a.length() < 8)a += " ";
		char a2[8];
		for (int i = 0; i < a.length(); i++)a2[i] = a[i];
		bitset<64> temp = charToBitset(a2);
		bitset<64> cipher=encrypt(temp);
	...
	//导入密文，进行解密，之后写入文本
	in.open("D://tests//des//plaintext.txt", ios::binary);
	out.open("D://tests//des//ciphertext.txt", ios::binary);
